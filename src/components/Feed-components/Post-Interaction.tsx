...
function PostInteractions({ initialLikes = 0, initialComments = [] }: PostInteractionsProps) {
  const heartOff = (
  ...
  const handleAddComment = (e?: React.FormEvent) => {
    ...
    if (!text) return;
    const comment: Comment = { user: "Tú", text };
  ...
  return (
    <div className="p-2.5 flex flex-col">
      <div className="flex flex-row items-center h-fit gap-0.5">
        ...
        <h2 id="like-counter" className={likedColors + " flex items-baseline text-lg ml-2"}>{likes}</h2>
      </div>

      <div className="flex justify-around mt-2">
        <button className={btnEffects + btnState} onClick={handleLike}>{liked ? "Liked" : "Like"}</button>
        <button className={btnEffects} onClick={toggleCommentBox}>Comentar</button>
      ...
      {showCommentBox && (
        <div className="mt-3 w-full">
          <form onSubmit={handleAddComment} className="flex gap-2 items-start">
            ...
            />
            <button type="submit" className="ml-2 bg-brand text-white px-4 py-2 rounded-lg">Enviar</button>
          </form>
